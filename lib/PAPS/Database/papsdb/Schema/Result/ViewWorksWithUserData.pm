use utf8;
package PAPS::Database::papsdb::Schema::Result::ViewWorksWithUserData;

=head1 NAME

PAPS::Database::papsdb::Schema::Result::ViewWorksWithUserData
...
__PACKAGE__->table('view_works_with_user_data');
__PACKAGE__->result_source_instance->is_virtual(1);

__PACKAGE__->result_source_instance->view_definition(q[
SELECT w.work_id as work_id, w.meta_work_id as meta_work_id,
  w.work_type_id as work_type_id, w.title as title, w.subtitle as subtitle,
  w.edition as edition, w.num_references as num_references,
  w.doi as doi, w.year as year,
  work_references_referencing_works.referencing_work_id as referencing_work_id,
  user_work_datas.read_timestamp as read_timestamp,
  user_work_datas.understood_rating as understood_rating,
  user_work_datas.approval_rating as approval_rating
FROM works w
LEFT JOIN user_work_data user_work_datas ON user_work_datas.work_id = w.work_id
  AND user_work_datas.user_id = ?
LEFT JOIN work_references work_references_referencing_works ON work_references_referencing_works.referenced_work_id = w.work_id
ORDER BY w.work_id
]);
